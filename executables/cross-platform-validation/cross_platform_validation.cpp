...
//
// Reads every JSON architecture from simulations/cedar, runs the two-phase
// protocol (500 steps stimulus ON, 500 steps stimulus OFF), and saves the
// field activation profiles to data/cedar/ as CSV files.
//
...
// retrieved by element lookup, and the LoopedTrigger is stepped a fixed number
// of times. The field activation (cv::Mat, CV_32F) is read out directly. No
// neural-field equations are reimplemented here.
//
...
int main(int argc, char* argv[])
{
    ...
    const fs::path sim_dir = (argc >= 2)
        ? fs::path(argv[1])
        : fs::path(R"(C:\dev-files\dynamic-field-theory-software\cross-platform-validation\simulations\cedar)");
    const fs::path out_dir = (argc >= 3)
        ? fs::path(argv[2])
        : fs::path(R"(C:\dev-files\dynamic-field-theory-software\cross-platform-validation\data\cedar)");
